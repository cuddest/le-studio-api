...
func GenerateAccessToken(secret string, subjectID uint, role, email string, ttl time.Duration) (string, error) {
	claims := Claims{Role: role, Email: email, RegisteredClaims: jwtv5.RegisteredClaims{Subject: fmt.Sprintf("%d", subjectID), IssuedAt: jwtv5.NewNumericDate(time.Now()), ExpiresAt: jwtv5.NewNumericDate(time.Now().Add(ttl))}}
...
func ParseAccessToken(secret, token string) (*Claims, error) {
	t, err := jwtv5.ParseWithClaims(token, &Claims{}, func(token *jwtv5.Token) (any, error) { return []byte(secret), nil })
	if err != nil {
		return nil, err
	}
